...
사용자가 서비스에 처음 진입하는 화면은 상품 약관을 가져오는 두 경로를 제공합니다. **상품 선택이 기본 흐름**이고 **문서 업로드는 보조 경로**입니다.

- **상품 선택 (기본, 상단)**
  - 금융기관과 상품명을 **드롭박스 + 검색창**으로 선택합니다.
  - 선택하면 서버에 미리 저장된 해당 상품의 약관서를 불러와 기존 추출·검증 파이프라인으로 처리합니다.
  - 금융기관·상품별 약관서는 **일배치 크롤러**가 서버단에서 수집해 **RDB에 미리 저장**해 둔다고 가정합니다. (MVP 가정)
- **문서 직접 업로드 (보조, 하단)**
  - 목록에 없는 상품은 PDF, JPG, PNG 등 문서를 직접 올릴 수 있습니다.
  - 업로드 영역은 화면 **하단**에 두어 상품 선택이 기본 흐름임을 시각적으로 나타냅니다.

어느 경로로 들어오든 이후 사실 추출·검증·규칙·계산·설명·리포트 흐름은 동일하게 동작하며, 사전 저장된 약관도 검증 없이 authoritative로 간주되지 않습니다.

## P0 데모 흐름

1. **상품 선택 또는 문서 업로드**
   - 기본 경로: 금융기관과 상품명을 드롭박스·검색창으로 선택해, 일배치로 수집·저장된 약관서를 불러옵니다.
   - 보조 경로(화면 하단): 목록에 없는 상품은 PDF, PNG, JPEG를 직접 업로드합니다.
